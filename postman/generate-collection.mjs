...
const saveTokensScript = {
  exec: [
    ...
    "const tokens = json.tokens || json.data?.tokens;",
    "if (tokens) {",
    "  pm.collectionVariables.set('accessToken', tokens.access.token);",
    "  pm.collectionVariables.set('refreshToken', tokens.refresh.token);",
    "}",
    "if (json.user) {",
    ...
    "    pm.collectionVariables.set('workspaceId', companyId);",
    "  }",
    "}",
    "if (json.activeWorkspaceId) pm.collectionVariables.set('workspaceId', json.activeWorkspaceId);",
    "if (json.data?.workspaceId) pm.collectionVariables.set('workspaceId', json.data.workspaceId);",
...
const routes = [
  ...
  },
  {
    ...
    item: [
      ...
      req('Refresh Token', 'POST', '/v1/auth/refresh-token', {
        headers: [{ key: 'refreshtoken', value: '{{refreshToken}}' }],
        test: true,
      }),
      req('Logout All Sessions', 'POST', '/v1/auth/logout-all', {
        headers: [{ key: 'refreshtoken', value: '{{refreshToken}}' }],
      }),
  ...
  },
  {
    ...
    item: [
      ...
      req('Logout All Sessions', 'POST', '/v1/tokens/logout-all', { headers: [{ key: 'refreshtoken', value: '{{refreshToken}}' }] }),
