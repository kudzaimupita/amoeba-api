...
describe('Error middlewares', () => {
  describe('Error converter', () => {
    ...
    test('should convert an Error without message to ApiError with default message of that http status', () => {
      ...
      expect(next).toHaveBeenCalledWith(
        expect.objectContaining({
          statusCode: error.statusCode,
          // message:JSON httpStatus[error.statusCode],
          isOperational: false,
        })
      );
    });
  ...
  describe('Error handler', () => {
    ...
    test('should include a stable error code when supplied', () => {
      const error = new ApiError(httpStatus.UNAUTHORIZED, 'Invalid credentials', true, '', 'AUTH_INVALID_CREDENTIALS');
      const res = httpMocks.createResponse();
      const sendSpy = jest.spyOn(res, 'send');

      errorHandler(error, httpMocks.createRequest(), res, jest.fn() as any);
