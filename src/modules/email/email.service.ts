...
export const sendEmail = async (to: string, subject: string, text: string, html: string): Promise<void> => {
  const msg: Message = {
    from: 'no-reply@servly.app' || config.email.from,
    to,
    subject,
    text,
    html,
...
export async function sendEmailPlugin(recipients: any, subject: any, attachments = <any>[], html: any, sender: any) {
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      port: 587,
      auth: {
        user: 'MS_52GJoA@trial-vz9dlemeqq14kj50.mlsender.net',
        pass: 'mssp.jVOyHxC.z86org8yqozlew13.g0zYMoW',
      },
    });
